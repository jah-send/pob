...
int main (void)
{
	...
	RGBFrame FrameFromCam;			//Struct to get the RGB components 
	...
	InitPOBEYE();		//initialize the POB-EYE (is coming from the POB-LIBRARY)
	InitLCD();			//initialize the POB-LCD128 (is coming from the POB-LIBRARY)
	InitPobProto();		//initialize POB-PROTO, the source code is at the end of this file

	
	GetPointerOnRGBFrame(&FrameFromCam); //get the pointer of the space memory reserved for the RGB components
							...
							{		
								if(IsCursorOnIcon(&TestMenu,&WhichIcon)) // test if the cursor is on an icon
								{
									...
									case 11:
										...
										{
											SetServoMotor(WhichIcon/2,ServPos[WhichIcon/2]);
										}
										else
										{
											SetServoMotor((WhichIcon/2)+2,ServPos[WhichIcon/2]);
										}
